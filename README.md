...
**Any time we choose to add multiple handlers in a single stage, we are forking the pipeline that many times.** When a fork occurs, all downstream stages are implicitly duplicated to exist in each stream and each transient record is cloned and emitted on all available downstream paths.

In the above example, because we have two functions in our first stage, the pipeline is forked into branches A and B such that all records from the generator are duplicated to traverse both paths. That means there are two copies of `exampleEnd`, one on each stream, and the sum of all records emitted from both `exampleEnd` functions is twice the number of records generated by `exampleGen`.

Downstream forks multiply and compound upstream branches. For example, a pipeline with three stages, each containing three functions, would produce 27 output streams.
...
-   Example of composing pipeline patterns with `InParallel` (Fan-out/Fan-in) `./cmd/pipeline_fan_out.go`
